...
export default function Navbar() {
  ...
  useEffect(() => {
    const screensize = () => setIsDesktop(window.innerWidth > 765);
    screensize();
    ...
    <div className="w-full ">
      {/* {isDesktop ? (
        ...
        <section className="w-full flex justify-center px-5 py-3">
          <div className="fixed top-4 w-[90%] max-w-4xl flex items-center justify-between px-6 py-3 border border-brand-orange rounded-2xl shadow-lgbackdrop-blur-md bg-black/40 z-50">
      ...
      <section className="w-full flex md:hidden justify-center px-5 py-3">
        <div className="fixed top-4 w-[90%] max-w-4xl flex items-center justify-between px-6 py-3 border border-brand-orange rounded-2xl shadow-lgbackdrop-blur-md bg-black/40 z-50">
          <nav className="flex w-full justify-between ">
            {navitems.map((n) => (
              <Link
      ...
      <div className="hidden md:flex fixed left-0 top-0 px-[50px] 2xl:px-[100px] items-center justify-between w-full h-[70px] shadow-sm backdrop-blur ">
        <section className="flex items-center gap-3">
          <Image src="/vercel.svg" alt="logo" width={30} height={30} />
          <h1 className="text-2xl font-semibold bg-gradient-to-r from-brand-red to-brand-orange bg-clip-text text-transparent">
            Samuel Eseigbe
